...
The full suite takes about an hour in the dev container (measured 2601s and
3897s), dominated by the nemotron-linprog replay tests over the 4 monthly
fixtures. Use a shell timeout of at least 4500000 ms when running it. If you
need to background it, detach with
`setsid nohup ... > /tmp/pytest_full.log 2>&1 < /dev/null &` — a plain `&` gets
killed when the opencode bash call returns.
