...
| `BATTERY_FOLLOW_BUDGET_MIN_FACTOR` | Minimum discharge budget for load-following intervals as a fraction of the maximum interval discharge. | `0.10` | Applied when the price spread to the future peak is large, so the battery conserves energy for the genuinely expensive periods. Set to `1.0` to effectively disable the cap (legacy behaviour). |
| `BATTERY_FOLLOW_BUDGET_SPREAD_FACTOR` | How aggressively the load-following discharge budget shrinks as the current import price sits below the most expensive remaining interval. | `2.5` | The planner assigns each interval a `discharge_budget_kwh` scaled by `1 - spread_factor * (peak_price - current_price) / peak_price`, clamped to `[min_factor, 1.0]`. Flat price days (small spread) keep budgets generous so stored solar covers night loads (e.g. EV charging) instead of forcing grid imports; volatile days with a real peak ahead tighten them. `run_often.py` caps load-following discharge (`follow`/`discharge_load` actions) to the remaining budget for the interval. Set to `0.0` to disable the spread scaling (constant `min_factor` floor). |
