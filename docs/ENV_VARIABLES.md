...
| `BATTERY_FOLLOW_BUDGET_MIN_FACTOR` | Minimum discharge budget for load-following intervals as a fraction of the maximum interval discharge. | `0.10` | The planner assigns each interval a `discharge_budget_kwh` that scales from this floor (cheapest prices) up to `1.0` (most expensive prices). `run_often.py` caps load-following discharge (`follow`/`discharge_load` actions) to the remaining budget for the interval, so the battery conserves energy during cheap periods (e.g. EV charging spikes) for higher-profit ones. Set to `1.0` to effectively disable the cap (legacy behaviour). |
