...
| Stage | Behaviour |
|-------|-----------|
| **Planning** | Battery power is 0 kW. No intentional charge or discharge. |
| **Real-time** | Proportional control adjusts battery power to drive net grid flow toward zero, with a 200W deadband to prevent hunting. If net grid power exceeds ±200W, the setpoint is moved to cancel the imbalance. |

**Permitted limits**: ±max_battery_kw (clamped to physical limits and fuse phase caps).
...
|--------|---------------|---------------------------|-------------------|-----------------|-----------------|
| `idle` | Both | Yes (opportunistic zero-grid) | Opportunistic | Opportunistic | ±max_kw, phase caps |
